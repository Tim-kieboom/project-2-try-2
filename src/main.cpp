...
bool printHTML(int ultrasoonSensor, bool REED, int* irArray, WiFiClient &client){
  ...
  uint8_t i = 0;
  while(irArray[i] != -1){
    client.println("<p>Infrarood sensor " + String(i+1) + ": </p>");
    client.println("<p>" + String(irArray[i]) + "</p>");
    client.println("<br>");
    i++;
  }

  client.println("<p>Ultrasonische Afstands Sensor: </p>");
  client.println("<p>" + String(ultrasoonSensor) + "CM</p>");
  
  client.println("<p>REED Sensor: </p>");
  client.println("<p>" + String((REED) ? "true" : "false") + "</p>");
